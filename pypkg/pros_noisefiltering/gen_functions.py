"""Generic functions for all types of operations for ease.

Mainly contains reused basic libaryies for less redundancy in src files.
"""
# %%
import pathlib
...
def plot_spect_comb2(graph_objlist,
    ...
    """Plot different signal power spectrums combined in one graph.

    This function plots the power spectrum diagram of an arbitray
    signals.The amplitute and frequency of the signals are calculated
    with signal.welch() function.
    Args:
    ...
    if kwargs.get('to_disk', None) is True:
        ...
        target_path = pathlib.Path('_temp_fig/{}.png'.format(
...
class Axis_titles:
    """Store titles for x and y axis."""

    def __init__(self,
                 x_title: str,
                 y_title: str) -> None:
        """Initialize a constructor to store the x and y labels."""
        self.x_title = x_title
        self.y_title = y_title


class Time_domain_data_cont():
    """A class for importing the x,y variables.

    Here we store the information we want to plot
    for the signal.
    """
...
def plot_signals(time_domain_sig,
    ...
    This function is used to plot signals in time
    domain from the old dataset.
    It was updated to use class objects as
    import for the x,y components of the signal and
    the axis titles instead of a variable oriented
    import (old plot_signals function)
...
# TODO change with Fft_New in org mode file is better.
# BUG
def fft_sig(signals,
            f0=2_000,
            fs=500_000):
    """Computes the fourier transform.
    For two seperate signals and returns the results
...
def plot_FFT(signals,
    ...
    """
    for objct, obj, ax_titles in zip(signals,
                                     info,
                                     axis_titles):
        ...
        ax2.loglog(objct.x, objct.filt_mag,
                   'orange',
...
def apply_filter(sig_r: np.ndarray, fs_hz: float, fc_hz=100, filt_order=2):
    """Overlapping and documented in basic processing class."""
...
def data_import(file_path: str,
    ...
    Returns:
        MATRIX_RAW (list): list of arrays transformed columns from dataset
    ...
    """
    f_1 = pd.HDFStore(
        path=f'{file_path}{file_name_of_raw}', mode='r')

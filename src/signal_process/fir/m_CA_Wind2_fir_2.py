...
class Fir_filter:
    def __init__(self, signals) -> None:
        ...
        self.fs_hz = int(
            1/signals._channel_data.properties['wf_increment'])
    ...
    def apply_fir(self, fc_hz):
        filtered, filt_time_int = fir_filter(self.raw,
                                             time=self.time_int,
                                             fs_hz=self.fs_hz,
                                             fc_hz=fc_hz)
    ...
    def get_spect_fir_output(self, fc_hz):
        x_filt, y_filt = spect(self.apply_fir(fc_hz=fc_hz),
                               FS=self.fs_hz)
        filt_type = 'FIR'
        return Graph_data_container(
            _x=x_filt, _y=y_filt,
